...
namespace WebCSharp.Areas.Admin.Controllers
{
    public class PostController : Controller
    {
        ...
        public ActionResult Index(string Searchtext, int? page)
        {
            ...
            IEnumerable<Post> items = _dbConect.Post.OrderBy(x => x.Id);
            if (!string.IsNullOrEmpty(Searchtext))
            {
                items = items.Where(x => x.Slug.Contains(Searchtext) || x.Title.Contains(Searchtext));
            }
        ...
        {
            ViewBag.Topic = new SelectList(_dbConect.Topic.ToList(), "Id", "Name");
            return View();
        }
        ...
        {
            var item = _dbConect.Post.Find(id);
            return View(item);
        }
        ...
        public ActionResult Edit(Post model)
        {
            if (ModelState.IsValid)
            {
                _dbConect.Post.Attach(model);
                model.Update_at = DateTime.Now;


                model.Slug = WebCSharp.Models.Commons.Filter.FilterChar(model.Title);
                _dbConect.Entry(model).Property(x => x.TopicId).IsModified = true;
                _dbConect.Entry(model).Property(x => x.Slug).IsModified = true;
                _dbConect.Entry(model).Property(x => x.Detail).IsModified = true;
                _dbConect.Entry(model).Property(x => x.Image).IsModified = true;
        ...
        public ActionResult DeleteAll(string ids)
        {
            ...
            if (!string.IsNullOrEmpty(ids))
            {
                ...
                if (items != null && items.Any())
                {
                    ...
                    {
                        var obj = _dbConect.Post.Find(Convert.ToInt32(item));
                        _dbConect.Post.Remove(obj);
                        _dbConect.SaveChanges();
                    }
                }
